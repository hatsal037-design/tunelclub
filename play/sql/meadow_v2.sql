-- 잔디밭 v2 — v1(meadow.sql, 코덱스, 운영 DB 적용됨) 위에 더한다. 기존 표·정책은 건드리지 않고 칸과 표만 보탠다.
-- 2026-09-24 햇살님 «공지·채팅·사진 올리기·투표·일정표·조 짜기(잘 섞이게 랜덤) 싹 다 구현해 봐».
-- 더하는 것: 채팅(chat) · 공지 기록(announcements) · 마피아 MVP 투표(mvp_votes) · 조 섞기(teams, team_history)
--           · 일정표 «지금 하는 중»(now_idx) · 사진 투표 결과 공개 시점(photo_reveal) · MVP 공개(mvp_open, mvp_reveal).
-- 좋알람은 넣지 않는다 — 플랫폼 규칙(5회 이상 참석)이라 1회 행사에서는 켤 사람이 없다.
begin;
alter table meadow_private.events add column if not exists now_idx integer not null default -1;
alter table meadow_private.events add column if not exists mvp_open boolean not null default false;
alter table meadow_private.events add column if not exists mvp_reveal boolean not null default false;
alter table meadow_private.events add column if not exists photo_reveal boolean not null default false;
alter table meadow_private.events add column if not exists teams jsonb not null default '[]' check(jsonb_typeof(teams)='array');
alter table meadow_private.events add column if not exists teams_title text not null default '' check(length(teams_title)<=40);
alter table meadow_private.events add column if not exists team_history jsonb not null default '[]' check(jsonb_typeof(team_history)='array');
create table if not exists meadow_private.chat (
...
create index if not exists meadow_chat_event on meadow_private.chat(event_id,id);
create index if not exists meadow_chat_member on meadow_private.chat(member_id);
create table if not exists meadow_private.announcements (
  id bigserial primary key,
...
create index if not exists meadow_announce_event on meadow_private.announcements(event_id,id);
create table if not exists meadow_private.mvp_votes (
  event_id uuid not null,
  member_id uuid not null,
...
create index if not exists meadow_mvp_target on meadow_private.mvp_votes(event_id,target_id);
alter table meadow_private.chat enable row level security;
alter table meadow_private.announcements enable row level security;
alter table meadow_private.mvp_votes enable row level security;
revoke all on all tables in schema meadow_private from public, anon, authenticated;
revoke all on all sequences in schema meadow_private from public, anon, authenticated;
